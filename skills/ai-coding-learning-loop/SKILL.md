...
1. Keep the coding task and the learning target separate. `engineering_task` says what must be built or changed; the user's one-sentence learning target says what they want to understand or apply afterward. Never invent a coding project merely from the learning target.
2. Inspect existing task context when available. If `/ownership start` already captured an explicit `engineering_task`, use it as authoritative scope unless the user explicitly changes it.
3. Treat the user's one-sentence learning target as the anchor. Refine it into concrete learning anchors in the Plan; do not make the user pre-author a curriculum.
...
6. `/ownership start` already presents a human-readable Learning Contract and waits for explicit acceptance before this Skill continues. Contract acceptance is not Plan approval and does not authorize implementation.
...
- On every resume, call `ownership_lifecycle` with `action: "status"` first. Use the returned `context` for the authoritative coding task, learning targets, work units, mode, learner profile, Gate policy, and latest Plan. Do not search the private evidence directory to rediscover the contract.
- Brief before planning: explain scope, interfaces, constraints, ownership, and verification for the contracted `engineering_task`.
- In Planning, refine the user's target into learning anchors and produce implementation steps, verification, and known risks.
- `submit_plan` is the Plan handoff. In DeepSeek Harness it opens the native Plan Review UI with the exact Plan markdown and records `APPROVE` or `REVISE` as direct user interaction evidence. Do not duplicate the same approval request in ordinary chat when `plan_review.channel` is `native-user-question`.
- If `submit_plan` returns `plan_review.channel: "direct-message-fallback"`, show the same Plan in chat and stop in `AWAITING_PLAN_REVIEW`; only a new direct user message may then be recorded with `record_plan_review`.
- If native Plan Review returns `REVISE`, stay in Planning, use transient `plan_review.feedback` when present, revise the Plan, and submit it again. Do not implement.
- If Plan Review returns `APPROVE`, call `start_work` immediately before implementation begins. Never edit implementation files or execute side-effectful tools before durable state enters an implementation phase.
...
2. `brief` with the contracted `work_unit_id` and the topics just taught;
3. `start_plan`, then `submit_plan` with all required fields: `schema_version`, `work_unit_id`, `implementation_steps`, `verification_plan`, `learning_anchors`, and `known_risks`;
