...
test('pinned Harness live acceptance is reproducible and provider-free', async () => {
  ...
  assert.match(workflow, /99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/)
  assert.match(workflow, /plugin --profile learning add/)
  assert.match(workflow, /latest-release-canary/)
  assert.match(workflow, /releases\?per_page=20/)
  ...
  assert.equal(report.upstream.commit, '99f6f02fecdb7dff40c3fbc9470f5907c29f74ca')
...
test('local testing keeps disposable evidence and credentials out of Git', async () => {
  ...
  assert.match(guide, /99f6f02fecdb7dff40c3fbc9470f5907c29f74ca/)
  ...
  assert.match(orchestrator, /DSH_HARNESS_REF/)
  assert.match(orchestrator, /pnpm\.cjs/)
  assert.match(orchestrator, /packageManagerBin/)
  assert.match(orchestrator, /delimiter/)
  assert.match(orchestrator, /mkdtempSync\(join\(tmpdir\(\), 'dsh-plugin-'\)\)/)
  assert.match(orchestrator, /process\.platform === 'win32' \? 'junction' : 'dir'/)
  assert.match(orchestrator, /'add', pluginAlias/)
  assert.doesNotMatch(orchestrator, /'add', pluginRoot/)
  assert.match(orchestrator, /rmSync\(aliasRoot, \{ recursive: true, force: true \}\)/)
